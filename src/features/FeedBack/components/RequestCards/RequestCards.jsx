...
function RequestCards() {
  ...
  const filterMyRequestFeedback = feedbacks.length > 0 ? feedbacks.filter((item, index) => (item.feedbackMainData.userIdFrom._id===userId&&item.feedbackMainData.feedbackType == "requested"
      ...
      isLoadingFeedback==true ? (
        ...
        filterMyRequestFeedback.length > 0 ? (
          filterMyRequestFeedback.map((item, index) => (
            <RequestCardItem key={item.feedbackMainData._id}  text={item.feedbackMainData.message}  image={image2} name={`${item.feedbackMainData.userIdTo.firstName} ${item.feedbackMainData.userIdTo.lastName}`} date={item.feedbackMainData.createdAt.substring(0,10)} cardId={item.feedbackMainData._id}/>
          ))
        ) : (
          (isLoadingFeedback==false&&filterMyRequestFeedback.length==0)?<div className='w-full flex flex-row justify-center'>
