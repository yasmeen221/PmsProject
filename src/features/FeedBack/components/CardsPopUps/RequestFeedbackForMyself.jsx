...
const schema = yup.object().shape({
    ...
    competencies: yup.array().min(1, 'Select at least one competency').of(
        yup.object().shape({
...
export default function RequestFeedbackForMyself({ OnClose }) {
    ...
    const user = useSelector(state => state.persistantReducer.userDataReducer.userData);
    ...
    const fetchUsernames = () => {
        axios.get(`${URL}/user/usernames`)
            .then(response => {

                const userData = response.data.data;

                const usernames = userData.usersNames.map(user => ({ value: user._id, label: user.username, teamId: user.team }));
    ...
    const fetchTeamLeader = (userId) => {
        axios.get(`${URL}/user/team-leader/${userId}`)
            .then(response => {
                const teamLeaderId = response.data.data.teamLeader._id;
    ...
    const fetchCompetencies = (teamId) => {
        axios.get(`${URL}/competency/teams-competencies/${teamId}`)
            .then(competencyResponse => {
                const competenciesData = competencyResponse.data.data.teamCompetencies.map(item => ({ value: item._id, label: item.name }));
    ...
    const handleSearchInputChange = (inputValue) => {
        if (inputValue) {
            const filteredUsernames = usernames.filter(user => user.label.toLowerCase().startsWith(inputValue.toLowerCase()));
            ...
            >
                <form onSubmit={handleSubmit(formSubmit)} className="  pb-4 ">
                        ...
                        <div className="pt-4">
                            ...
                            <Select
                                ...
                                onChange={(selectedOption) => {
                                    ...
                                    fetchTeamLeaderAndCompetencies(selectedOption.value, selectedOption.teamId);
                                }}
                            ...
                            {errors.userIdTo && <p className="text-red-500">{errors.userIdTo.message}</p>}
                    ...
                    <div className="flex items-center justify-end border-t border-gray-200 py-3 mx-1">
                        <Button className="px-6 py-3.5 text-fontColor-whiteBaseColor" buttonText="Request" type="submit" />
                    </div>
                </form>
            </FormPopUp>
