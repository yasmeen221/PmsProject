...
function ManageTeamsForm() {
  ...
  const { data: users, isUsersError,isUsersSuccess,isUsersLoading,usersError } = useGetUsersQuery();
  const[editTeam,{isLoadingEditTeam,isErrorEditTeam,errorEditTeam,isSuccessEditTeam}]=useEditTeamMutation()
  ...
  const onSubmit = (data) => {
    ...
    // addTeam(data)
    itemToEdit.teamName?editTeam({_id:itemToEdit._id,...data}):addTeam(data).then((data)=>{console.log(data)})
    console.log(isError)
    console.log(error)
  };
  return (
          ...
          <div className="my-2 w-full">
            ...
            <div className="relative mt-2">
              ...
              >
                ...
                { !isUsersLoading&&!isUsersError&& users?.data.users.map((team, index) => {
                  return (
