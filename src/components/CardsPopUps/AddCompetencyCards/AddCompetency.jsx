...
function AddCompetency({ open }) {
    ...
    return (
        <>
            <HandelPopUp isOpen={isPopupOpen} ClosePop={handleClosePopup} TitlePopUp="Add New Competency" >
                <div style={{ width: "35vw", maxHeight: "65vh", overflowY: 'auto', scrollbarWidth: 'none' }} className='px-1'>
                    ...
                    <div className='w-full inline-flex justify-between   items-center my-2'>
                        <div>
                            ...
                            <p className='text-fontColor-placeHolderColor  text-body1Size' >By default competency assign to everyone</p>
                        </div>
                        <div>
                            <label className="inline-flex items-center cursor-pointer">
                                <input type="checkbox" value="" className="sr-only peer" onChange={(e) => { setAddToggle(e.target.checked) }} />
                    ...
                    <div className='my-2'>
                        <Header text="Levels" />
                        <p className='text-fontColor-placeHolderColor  text-body1Size' >Select Levels to customize competency descriptions for each career path level.</p>
                        <div className="relative mt-2">
                            <select onChange={(e) => setLevel(level => e.target.value)} className={`block appearance-none w-full bg-white border-0    py-2.5 px-2 ring-1 ring-inset ring-fontColor-outLineInputColor  rounded-buttonRadius shadow-sm   focus:shadow-outline focus:ring-2 focus:ring-buttonColor-baseColor focus:outline-none ${level == "" ? 'text-fontColor-placeHolderColor' : 'text-fontColor-blackBaseColor'} `} >
                                <option value=""  >select category</option>
                                <option value="1">Option 1</option>
                                <option value="2">Option 2</option>
                                <option value="3">Option 3</option>
